...
#include "ui_znodiumcontroldialog.h"
...
std::list<std::string> ZNodiumControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZNodiumControlDialog::listMints;

ZNodiumControlDialog::ZNodiumControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZNodiumControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZNodiumControlDialog::~ZNodiumControlDialog()
{
    delete ui;
}

void ZNodiumControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNodiumControlDialog::updateList()
{
...
void ZNodiumControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNodiumControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZNodium_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZNodiumControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZNodiumControlDialog::GetSelectedMints()
{
...
void ZNodiumControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
